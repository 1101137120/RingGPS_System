...
exports.postFindTagPosition = function(req, res,next) {
	...
	else
	{
		...
		"SELECT * FROM time_line_animal,tag_animal WHERE  time_line_animal.tag_uid=tag_animal.tag_uid and tag_animal.id='"+tag_id+"' order by create_at  desc limit 1";
		...
		tagInstance.query(sql,function(err, rows, fields) {
			...
			else
			{
				...
				time_line_record.reader_name = rows['0'].reader_name;
				time_line_record.create_at = rows['0'].create_at;
				if(rows['0'].state===0){
					time_line_record.state="待開啟";
				
				}else{
					time_line_record.state="開啟";
				}
