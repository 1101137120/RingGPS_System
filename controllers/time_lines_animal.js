var Time_lineInstance= require('../models/time_line_animal'); 

// Handle /2.4/v1/time_lines for POST
exports.posttime_line_animal = function(req, res,next) {
	var customErr = new Error();
	var errorMessages = new Array();
	customErr.status = 400;
	var time_lineInstance = new Time_lineInstance();		
	var tag_uid_data = false;
	...
	//if(typeof req.body.created_at === "undefined")errorMessages.push("Missing 'created_at' field");
	if(typeof req.body.tag_uid === "undefined")errorMessages.push("Missing 'tag_uid' field");
	...
	else
	{
		...
		var sql = "SELECT tag_uid,id from time_line_animal"
		time_lineInstance.query(sql,function(err,rows,fields){				
				if(rows.length===0){
					...
					time_lineInstance.set('tag_uid', tag_uid);
					time_lineInstance.set('create_at', create_at);
			...
			for(var i=0;i<rows.length;i++){
				if(tag_uid===rows[i].tag_uid){
						 tag_uid_data = true;
						 var tag_id = rows[i].id;
				...
				if(tag_uid_data){
					if(tag_uid !== "")
					time_lineInstance.set('id',tag_id);
					time_lineInstance.set('reader_name', reader_name);
					time_lineInstance.set('create_at', create_at);
					time_lineInstance.set('tag_uid', tag_uid);
					time_lineInstance.save(function(err){
					//time_lineInstance.query(sql,function(err, rows, fields) {
						...
						{	
							...
							var apiOutput = {};
							apiOutput.status = "success";
							apiOutput.message = "update time_line established";
							res.json(apiOutput);		
							console.log(JSON.stringify(apiOutput));
						}
					});	
		...
		/*else
		{
			...
			time_lineInstance.query(sql,function(err,rows,fields){
				...
				{*/
					...
					//zvar sql = "INSERT INTO time_line_animal  (reader_name,tag_uid,create_at) VALUES ('"+reader_name+"','"+tag_uid+"','"+create_at+"')";
